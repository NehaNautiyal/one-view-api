...
class Scrape {
    ...
    scrapeTotalReviews(totalReviewCount, averageStarRating, ASIN, keywordString) {
        ...
        axios.get("https://www.amazon.com/product-reviews/" + ASIN + "/ref=cm_cr_arp_d_viewopt_srt?ie=UTF8&reviewerType=avp_only_reviews&pageNumber=1&sortBy=recent")
            ...
            }).then(response => { return this.scrapeReviews(totalReviewCount, ASIN, keywordString) })
            .catch(err => console.log(err));
    }


    scrapeReviews(totalReviewCount, ASIN, keywordString) {
        ...
        for (let page = 1; page <= (Math.ceil(totalReviewCount / 10)); page++) {
            ...
            axios.get(queryURL).then((response) => {
                ...
                if (numReviews === parseInt(totalReviewCount)) {
                    return this.deleteAllInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews, keywordString);
                }
    ...
    deleteAllInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews, keywordString) {
        // grabs all of the reviews to delete
        db.Review.deleteMany()
            .then((reviews) => {
                console.log("All deleted");
                return this.inputReviewInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews, keywordString);
            })
            .catch(error => {
                console.log(error);
            });
    ...
    inputReviewInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews, keywordString) {
        ...
        if (parseInt(numReviews) === parseInt(numDocuments)) {
            ...
            if (keywordString) {
                ...
                if (keywordArray.length === 3) {
                    ...
                    watson.analyzeReviews(reviewStr, keyword1, keyword2, keyword3, function (analysis) {
                        return analysis;
                    ...
                    keyword1 = keywordArray[0];
                    keyword2 = keywordArray[1];
                    watson.analyzeReviews(reviewStr, keyword1, keyword2, function (analysis) {
                        return analysis;
                        // console.log(analysis);
                    });
                } else if (keywordArray.length === 1) {
                    keyword1 = keywordArray[0];
                    watson.analyzeReviews(reviewStr, keyword1, function (analysis) {
                        return analysis;
                        // console.log(analysis);
                    });
                }
