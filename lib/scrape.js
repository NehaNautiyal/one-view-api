...
class Scrape {
    ...
    scrapeTotalReviews(totalReviewCount, averageStarRating, ASIN) {
     ...
     }).then(response => {this.scrapeReviews(totalReviewCount, ASIN)})
    }
        ...
        for (let page = 1; page <= (Math.ceil(totalReviewCount / 10)); page++) {
            ...
            axios.get(queryURL).then((response) => {
                ...
                if (numReviews === parseInt(totalReviewCount)) {
                    this.deleteAllInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews);
                }
    ...
    deleteAllInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews) {
        ...
        db.Review.deleteMany()
            .then((reviews) => {
                ...
                this.inputReviewInMongo(amazonReviewText, amazonReviewDate, amazonReviewTitle, numReviews);
            })
